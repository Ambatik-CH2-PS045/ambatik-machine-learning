...
# model = tf.keras.models.load_model('gs://ambatik_bucket/ambatik_model/ambatik-model.h5')
app = Flask(__name__)
...
def finds(filename):
	...
	vals = ['cendrawasih', 'geblek renteng', 'insang', 'kawung', 'lasem', 'mega mendung', 'nitik', 'parang', 'poleng', 'pring sedapur', 'sekar jagad', 'simbut', 'sogan', 'tambal', 'truntum']
...
def predictCloudStorage(filename):
    ...
    gcs_url = f'https://storage.googleapis.com/ambatik_bucket/batik_prediction_photo/{filename}'
    ...
    img_width, img_height = 256, 256
    img = tf.keras.utils.load_img(local_path)
    width = img.size[0]
    height = img.size[1]
    ...
    if(width != height):
        if(width > height):
            diff= width - height
            start = diff // 2
            img = img.crop((start, 0, start + height , height))
        else:
            diff= height - width
            start = diff // 2
            img = img.crop((0,start,  width , start + width))
